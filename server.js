...
const twitchToken = "Bearer " + process.env.TWITCHTOKEN;
...
app.post("/eventsub", async (req, res) => {
  ...
  if (true === verifyMessage(hmac, req.headers[TWITCH_MESSAGE_SIGNATURE])) {
    ...
    // await twitchTokenRefresh();

    // TODO: turn this into a function
    // Check subscription status to yourself
    const subStatus = await axios
      .get(twitchSubUrl, {
        headers: {
      ...
      .then((res) => {
        return res.data && res.data.data.some((sub) => (sub.condition.broadcaster_user_id == myId))        
      })
      .catch((err) => console.log("check sub status error: " + err));
    console.log("status is: " + subStatus);
    ...
    if (MESSAGE_TYPE_NOTIFICATION === req.headers[MESSAGE_TYPE]) {
      ...
      const msg = "I went live on twitch!";
      await postTweet(msg);
